...
compose=(docker compose -f compose.yaml -f compose.override.yaml -f compose.acceptance.yaml)
proxy_address=${ACCEPTANCE_PROXY:-127.0.0.1:2080}
target=3d4017c3e843895a92b70aa74d1b7ebc9c982ccf2ec4968cc0cd55f12af4660c.ygg
restore() {
  "${compose[@]}" up -d --no-deps overlay-node overlay-http yggstack
}
trap restore EXIT
check_failure() {
...
"${compose[@]}" stop overlay-http overlay-node
check_failure
restore
for attempt in {1..40}; do
  if curl -fsS --max-time 3 --noproxy '' -x "http://${proxy_address}" "http://${target}/marker" >/dev/null 2>&1; then break; fi
