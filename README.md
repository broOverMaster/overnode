...
`internet`. CONNECT, цепочка через upstream и overlay-пересылка пока не реализованы
(`501`). Обычные HTTP-запросы получают `400`, CONNECT для специальных маршрутов — `405`.
